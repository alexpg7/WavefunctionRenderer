...
// Legendre P_l^m(x)
template<int l, int m> static float legendreP(float x)
{
	float pmm = 1.0f;
...
// Spherical harmonic
template<int l, int m> static std::complex<float> Ylm(float theta, float phi)
{
	int abs_m = std::abs(m);
...
// Hidrogen atom
template<int n, int l> static float Rnl(float r)
{
	float rho = 2.0f * r / n;
	int k = n - l - 1;
